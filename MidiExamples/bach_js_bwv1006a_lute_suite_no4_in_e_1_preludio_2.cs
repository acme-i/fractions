...
namespace fractions.examples
{
    ...
    internal class bach_js_bwv1006a_lute_suite_no4_in_e_1_preludio_2 : ExampleBase
    {
        ...
        private void Play()
        {
            ...
            {
                resultTimes.AddRange(shift(times, shifter.Next(), multen.Next()));
            }
            ...
            Clock.MaxPitch(Pitch.G5);
            Clock.MinPitch(Pitch.C3);
        ...
        private void PlayEchos(int i, int max)
        {
            ...
            for (var j = i; j < max; j++)
            {
                ...
                var nt = new NoteOnOffMessage(OutputDevice, ch, (Pitch)note.Note + 12, vol.Next() * scalers.Next(), (notsClone.Current().Time + notesTimes.Next()) / div, Clock, noteDurs.Next(), pm.Next());
                nt.BeforeSendingNoteOnOff += m =>
                {
                    OutputDevice.SendProgramChange(m.Channel, instr.Next());
                };
